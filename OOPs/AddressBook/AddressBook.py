import json
import sys  # importing this for accessing objects
...
class AddressBook:
    ...
    def create(self):
        self.list = {'data': []}  # append data in list
        # file = input('Enter the file which you want to create: \n')
    ...
    def add(self):
        ...
        if not Fname.isalpha():  # isalpha()- a-z A-Z
        ...
        if not mob.isnumeric():  # isalnum()- 0-9
        ...
        if not area.isalnum():  # isalnum()- a-z 0-9
        ...
        if not city.isalpha():  # isalpha()- alphabets a-z
    ...
    def update(self):      # function to update the data in existing file
        flag = update = False
        if len(self.list['data']) >= 1:
            ...
            for i in range(len(self.list['data'])):
                if (self.list['data'][i]['Fname'] == self.list['data'][i]['Fname']) and (
                    ...
                    flag = True
                    if flag:
                        op = int(input("Select AnyOne Option to update your Profile\n 1 Mobile \n 2 Area \n 3 "
                                       "Zipcode \n 4 City \n 5 State \n "))
                        if op == 1:
    ...
    def remove(self):         # function for removing data
        if len(self.list['data']) >= 1:
            Fname = input("Enter the First Name ")
            Lname = input("Enter the Last Name ")
            for i in range(len(self.list['data'])):
                if (str(self.list['data'][i]['Fname']).casefold() == Fname.casefold()
                    ...
                    # Casefold() - converts all characters to lower case
